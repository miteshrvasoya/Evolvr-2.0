...
Provide an improved prompt.`;
  ...
  app.get('/content/ungenerated-prompts', async (request: any, reply) => {
    ...
    // For each content idea, get the agent run that generated it via strategy_version_id:
    // We look at agent_runs whose time overlaps with the content idea creation
    // and correlate via the strategy version to get a "generated by run" grouping.
    const strategyVersionIds = [...new Set(rawIdeas.map((r: any) => r.strategyVersionId).filter(Boolean))];

    // Get agent runs that were active around the time content was generated
    // (content_ideas are generated within an agent_run context)
    const agentRuns = strategyVersionIds.length > 0 ? await sql`
      ...
      FROM agent_runs ar
      JOIN agent_steps ast ON ast.agent_run_id = ar.id
      JOIN strategy_versions sv ON sv.social_account_id = ar.social_account_id
      WHERE ar.social_account_id = ${accountId}
        AND sv.id = ANY(${strategyVersionIds as any})
        -- The run must have started before or at the strategy version creation
        AND ar.started_at <= sv.created_at + INTERVAL '1 hour'
      ORDER BY ar.id, ar.started_at DESC
    ` : [];

    // Map strategy_version_id → agent_run
    const svToRun = new Map<string, any>();
    for (const run of agentRuns) {
      if (!svToRun.has(run.strategyVersionId)) {
        svToRun.set(run.strategyVersionId, run);
      }
    }
    ...
    for (const idea of rawIdeas) {
      const run = svToRun.get(idea.strategyVersionId);
