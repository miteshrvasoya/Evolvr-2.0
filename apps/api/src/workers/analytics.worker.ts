...
import { AnalyticsAgent } from '../modules/agent/analytics.agent.js';
...
export const createAnalyticsWorker = () => {
  ...
  const worker = new Worker('analytics', async (job: Job) => {
    ...
    try {
      console.log(`[AnalyticsWorker] Processing job ${job.id} for account ${socialAccountId}`);

      await analyticsAgent.runAnalytics(socialAccountId, agentRunId, attemptNumber, maxAttempts);

      // Once analytics is fetched, drop back into orchestrator
      ...
      console.error(`[AnalyticsWorker] Failed job ${job.id}`, err);
